(() => {
  ...
  const CLASSES = ["Win32_ComputerSystem", "Win32_PhysicalMemory"];
  const TOTAL_PHYSICAL = "17179869184";
  ...
  function hookClassObject(object, className) {
    ...
    Interceptor.attach(getMethod, {
      ...
      onLeave(retval) {
        ...
        if (this.property !== "TotalPhysicalMemory" && this.property !== "Capacity") {
          return;
        }

        Agent.writeVariantAuto(this.value, TOTAL_PHYSICAL);
        ...
        Agent.triggered(TAG, "wmi", className, this.caller.toString(), {
          ...
          current: { [this.property]: TOTAL_PHYSICAL },
        });
      },
    });
  }
